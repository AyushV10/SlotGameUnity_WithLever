...
namespace SlotFunctionality
{
    ...
    public class SlotGameManager : MonoBehaviour
    {
        ...
        [SerializeField] private BonusManager bossManager;
        ...
        [SerializeField] private GameObject blockerSpinButton;
        [SerializeField] private GameObject blockerBetIncButton;
        [SerializeField] private GameObject blockerBetDecButton;
        [SerializeField] private GameObject SlotGameTitle2;
        ...
        private void WheelSpinning()
        {
            ...
            if (Time.time >= reelSpinTimestamp&& toleranceValue < 0.01f)
            {
                ...
                {
                    isSpinning = false;
                    blockerSpinButton.SetActive(false);
                    blockerBetIncButton.SetActive(false);
                    blockerBetDecButton.SetActive(false);
                    spinAudioSource.Stop();
                    matchMaker.CheckForMatches(symbols, slotLayoutManager);
                    var bonusModes = matchMaker.GetBonusModes();

                    if (bonusModes.Contains(BonusMode.BonusBattle))
                        EventManager.currentManager.AddEvent(new BonusSummon());
        ...
        public void StartSpin()
        {
            if(!scoreManager.TryPayForSpin())
                return;
            blockerSpinButton.SetActive(true);
            blockerBetIncButton.SetActive(true);
            blockerBetDecButton.SetActive(true);
            matchMaker.ClearLines();
            SlotGameTitle2.SetActive(false);
